...
def random_crop_slices(origin_size, target_size):
    ...
    assert origin_size[0] >= target_size[0] and origin_size[1] >= target_size[1], (
        f"actual size: {origin_size}, target size: {target_size}"
    )

    offset_y = torch.randint(
        0, origin_size[0] - target_size[0] + 1, (1,)
    ).item()  # range: 0 <= value < high
    offset_x = torch.randint(0, origin_size[1] - target_size[1] + 1, (1,)).item()

    return slice(offset_y, offset_y + target_size[0]), slice(
        offset_x, offset_x + target_size[1]
    )
...
class PhraseCut(object):
    def __init__(
        ...
        if remove_classes is None:
            ...
            if remove_classes[0] == "pas5i":
                ...
                avoid = [
                    PASCAL_5I_SYNSETS_ORDERED[i]
                    for i in range(20)
                    if i + 1 not in PASCAL_5I_CLASS_IDS[subset_id]
                ]
                ...
                avoid = [
                    c for class_set in PASCAL_VOC_CLASSES_ZS[:stop] for c in class_set
                ]
                print(avoid)
                ...
                keep_sids = [
                    ...
                    if all(x not in avoid for x in get_data(i)["phrases"][j].split(" "))
                ]
            ...
            if keep_sids is None:
                all_lemmas = [
                    s
                    for ps in avoid
                    for s in traverse_lemmas_hypo(wordnet.synset(ps), max_depth=None)
                ]
                ...
                remove_sids = set(
                    (i, j)
                    for (i, j), phrase in zip(self.sample_ids, phrases)
                    if any(l in phrase for l in all_lemmas_m)
                    or len(
                        set(wnl.lemmatize(w) for w in phrase.split(" ")).intersection(
                            all_lemmas_s
                        )
                    )
                    > 0
                ...
                keep_sids = [
                    (i, j) for i, j in self.sample_ids if (i, j) not in remove_sids
                ]
        ...
        samples_by_phrase = [
            (self.refvg_loader.get_img_ref_data(i)["phrases"][j], (i, j))
            for i, j in self.sample_ids
        ]
        samples_by_phrase = sorted(samples_by_phrase)
        samples_by_phrase = groupby(samples_by_phrase, key=lambda x: x[0])

        self.samples_by_phrase = {
            prompt: [s[1] for s in prompt_sample_ids]
            for prompt, prompt_sample_ids in samples_by_phrase
        }
        ...
        if self.only_visual:
            ...
            self.sample_ids = [
                ...
                if len(
                    self.samples_by_phrase[
                        self.refvg_loader.get_img_ref_data(i)["phrases"][j]
                    ]
                )
                > 1
            ]
        ...
        sizes = [
            self.refvg_loader.get_img_ref_data(i)["gt_boxes"][j]
            for i, j in self.sample_ids
        ]
        image_sizes = [
            self.refvg_loader.get_img_ref_data(i)["width"]
            * self.refvg_loader.get_img_ref_data(i)["height"]
            for i, j in self.sample_ids
        ]
        # self.sizes = [sum([(s[2] - s[0]) * (s[3] - s[1]) for s in size]) for size in sizes]
        self.sizes = [
            sum([s[2] * s[3] for s in size]) / img_size
            for size, img_size in zip(sizes, image_sizes)
        ]
        ...
        self.sample_ids = [
            ...
            if self.sizes[i] > min_size
        ]

        self.base_path = join(
            expanduser(
                "/data8/stable-diffusion/ldm/data/third_party/PhraseCutDataset/data/VGPhraseCut_v0/images/"
            )
        )
    ...
    def load_sample(self, sample_i, j):
        ...
        for polys in polys_phrase0:
            for poly in polys:
                ...
                masks += [
                    polygon2mask((img_ref_data["height"], img_ref_data["width"]), poly)
                ]

        seg = np.stack(masks).max(0)
        img = np.array(
            Image.open(join(self.base_path, str(img_ref_data["image_id"]) + ".jpg"))
        )

        min_shape = min(img.shape[:2])

        if self.aug_crop:
            sly, slx, exceed = find_crop(
                seg, (min_shape, min_shape), iterations=50, min_frac=0.05
            )
        else:
        ...
        seg = nnf.interpolate(seg, (self.image_size, self.image_size), mode="nearest")[
            0, 0
        ]
        img = nnf.interpolate(
            img, (self.image_size, self.image_size), mode="bilinear", align_corners=True
        )[0]
    ...
    def __getitem__(self, i):
        ...
        if self.with_visual:
            ...
            if (
                phrase in self.samples_by_phrase
                and len(self.samples_by_phrase[phrase]) > 1
            ):
                ...
                if self.mask in {"separate", "text_and_separate"}:
                    ...
                    masked_img_s = torch.from_numpy(
                        blend_image_segmentation(
                            img_s, seg_s, mode=mask_mode, image_size=self.image_size
                        )[0]
                    )
                ...
                if self.mask in {"separate", "text_and_separate"}:
                    ...
                    vis_s = add_phrase + [
                        vis_s,
                        torch.zeros(*vis_s.shape[1:], dtype=torch.uint8),
                        False,
                    ]
        ...
        if self.mask == "text":
            ...
            return {
                "image": img,
                "caption": vis_s[0],
                "visual_query": vis_s[1],
                "valid_visual_query": vis_s[2],
                "mask": seg,
                "mask_batch": torch.zeros(0),
                "cls_batch": i,
            }
