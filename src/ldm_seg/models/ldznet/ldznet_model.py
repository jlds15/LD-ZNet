...
from ldm_seg.modules.distributions.distributions import (
    normal_kl,
    DiagonalGaussianDistribution,
)
from ldm_seg.models.autoencoder import (
    VQModelInterface,
    IdentityFirstStage,
    AutoencoderKL,
)
...
class DDPM(pl.LightningModule):
    ...
    def __init__(
        ...
        super().__init__()
        assert parameterization in ["eps", "x0", "segmentation"], (
...
class LatentSeg(DDPM):
    ...
    def forward(self, x, c, mask=None, *args, **kwargs):
        t = torch.randint(
            0, 1, (x.shape[0],), device=self.device
        ).long()  # t should always be 0 for this experiment
        if self.model.conditioning_key is not None:
...
class DiffusionWrapper(pl.LightningModule):
    ...
    def forward(
        self, x, t, sd_features, c_concat: list = None, c_crossattn: list = None
    ):
