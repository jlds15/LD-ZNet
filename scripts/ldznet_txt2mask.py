...
class Script(scripts.Script):
    ...
    def ui(self, is_img2img):
        ...
        plug = gr.HTML(
            label="plug",
            value='<div class="gr-block gr-box relative w-full overflow-hidden border-solid border border-gray-200 gr-panel"><p>If you like my work, please consider showing your support on <strong><a href="https://patreon.com/thereforegames" target="_blank">Patreon</a></strong>. Thank you! &#10084;</p></div>',
        )
    ...
    def run(
        ...
        def process_mask_parts(these_preds, these_prompt_parts, mode, final_img=None):
            for i in range(these_prompt_parts):
                ...
                (thresh, bw_image) = cv2.threshold(
                    gray_image, mask_precision, 255, cv2.THRESH_BINARY
                )
        ...
        def load_model_from_config(config, ckpt, verbose=False):
            ...
            config.model.params.sd_features_stage_config.params.path = (
                config.model.params.sd_features_stage_config.params.path.replace(
                    "..", "extensions/LD-ZNet"
                )
            )
            config.model.params.sd_features_stage_config.params.config = (
                config.model.params.sd_features_stage_config.params.config.replace(
                    "..", "extensions/LD-ZNet"
                )
            )
            ...
            m, u = model.load_state_dict(
                ldm_weights_keys_updated, strict=False
            )  # loading the both stages of LDM along with the clip text encoder
            ...
            m, u = model.load_state_dict(
                sd, strict=False
            )  # loading just the segmentation model
        ...
        def get_mask():
            ...
            t = (
                torch.randint(0, 1, (init_image.shape[0],)).cuda().long()
            )  # t should always be 0 for this experiment
            ...
            with torch.no_grad():
                with model.ema_scope():
                    ...
                    sd_features = model.encode_sd_features_stage(
                        init_latent, c
                    )  # , timestep=400)
            ...
            if mask_padding > 0:
                ...
                final_img = center_crop(
                    final_img, p.init_images[0].width, p.init_images[0].height
                )
        ...
        model = load_model_from_config(
            config=config, ckpt="extensions/LD-ZNet/checkpoints/LDZNet_txt2mask.ckpt"
        )
